...
function localTime(d = new Date()) { return d.toLocaleTimeString([], {hour:'2-digit', minute:'2-digit'}); }
...
function renderEntries(){
  ...
  el.innerHTML = rows.map(e => `
    <article class="entry">
      <div class="entry-top">
        <div><div class="entry-title">${escapeHTML(e.domain)} · ${escapeHTML(e.type)}</div><div class="entry-meta">${escapeHTML(e.date)} ${escapeHTML(e.time)} ${e.quantity ? '· '+escapeHTML(e.quantity)+' '+escapeHTML(e.unit) : ''} ${e.score ? '· score '+escapeHTML(e.score) : ''}</div></div>
...
function renderNarratives(){
  ...
  el.className='entry-list';
  el.innerHTML = state.narratives.map(n => `<article class="entry"><div class="entry-top"><div><div class="entry-title">${escapeHTML(n.title)}</div><div class="entry-meta">${escapeHTML(n.date)} ${escapeHTML(n.time)}</div></div><button onclick="deleteRecord('narratives','${n.id}')">Delete</button></div><p>${escapeHTML(n.body)}</p></article>`).join('');
}
